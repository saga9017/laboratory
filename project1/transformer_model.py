...
class Encoder(nn.Module):
    def __init__(self, hidden_dim=300, hidden_dim_=512):  # default=512
        ...
        self.WQ_e_L1 = Parameter(torch.randn(hidden_dim_, hidden_dim))
        self.WK_e_L1 = Parameter(torch.randn(hidden_dim_, hidden_dim))
        self.WV_e_L1 = Parameter(torch.randn(hidden_dim_, hidden_dim))
        self.WO_e_L1 = Parameter(torch.randn(hidden_dim_, hidden_dim))

        self.W1_e = Parameter(torch.randn(hidden_dim, 4 * hidden_dim))
        self.b1_e = Parameter(torch.randn(4 * hidden_dim))
        self.W2_e = Parameter(torch.randn(4 * hidden_dim, hidden_dim))
        self.b2_e = Parameter(torch.randn(hidden_dim))

        self.gamma_a1 = Parameter(torch.ones((8, 1)))
        self.beta_a1 = Parameter(torch.zeros((8, 1)))

        self.gamma1 = Parameter(torch.tensor(1.0))
        self.beta1 = Parameter(torch.tensor(0.0))
        self.gamma2 = Parameter(torch.tensor(1.0))
        self.beta2 = Parameter(torch.tensor(0.0))
    ...
    def ffn1(self, x):  # feed forward network   x : (batch_size, input_len, hidden)
        ...
        relu = torch.max(torch.zeros_like(linear1), linear1)  # (batch_size, input_len, 4*hidden)
...
class transformer(nn.Module):
    def __init__(self, word_dim2, hidden_dim=300, hidden_dim_=512, label_smoothing=0.1):  # default=512
        ...
        self.V_e=Parameter(torch.randn(hidden_dim, 1))
        self.V_d = Parameter(torch.randn(max_len, word_dim2))
    ...
    def predict(self, x):
        ...
        x10 = self.softmax(norm_forward(x9.transpose(1, 2).matmul(self.V_d), self.gamma, self.beta)).view(x9.shape[0],
                                                                                                          -1)
        output=torch.tensor([re_label_dic[i.item()] for i in torch.argmax(x10, 1)])
        ...
        for epoch in range(nepoch):
            ...
            for i in range(total_step):
                ...
                lrate = math.pow(self.hidden, -0.5) * min(math.pow(num_examples_seen, -0.5),
                                                          num_examples_seen * math.pow(40, -1.5))
