...
class Multi_head_attention(nn.Module):
    def __init__(self, hidden_dim=300, hidden_dim_=1024, dropout=Dropout):
        super().__init__()
    ...
    def forward(self, en, de, mask, dropout=False):  # x : (input_len, hidden_dim)
        ...
        q = self.w_qs(de).view(len_d0, len_d1, -1, 16).permute(3, 0, 1, 2)
        k = self.w_ks(en).view(len_e0, len_e1, -1, 16).permute(3, 0, 2, 1)
        v = self.w_vs(en).view(len_e0, len_e1, -1, 16).permute(3, 0, 1, 2)
        ...
        a = torch.cat((head3[0], head3[1], head3[2], head3[3], head3[4], head3[5], head3[6], head3[7],
                       head3[8], head3[9], head3[10], head3[11], head3[12], head3[13], head3[14], head3[15]), 2)
...
class transformer(nn.Module):
    ...
    def train_with_batch(self, batch, batch_dev, batch_test, evaluate_loss_after=5):
        ...
        for epoch in range(nepoch):
            ...
            torch.save(model.state_dict(), 'transformer_big_saved_model/checkpoint'+str(epoch))
            print('saved')
        return last_loss

torch.manual_seed(10)
batch, key_dic, y_number, batch_dev, batch_test, label_dic= generate_batch(MAX_TOKEN)
torch.save(label_dic, 'transformer_big_label_dic')
torch.save(key_dic, 'transformer_big_key_dic')
torch.save(y_number, 'transformer_big_y_number')
