...
embedded_matrix=torch.from_numpy(np.concatenate((embedded[0], np.zeros((1,300))), 0))
w2i=embedded[1]
...
def data():
    return torch.tensor(X_train_padding), torch.tensor(y_train), max(y_train), label_dic
...
def data_dev():
    return torch.tensor(X_dev_padding), torch.tensor(y_dev)
