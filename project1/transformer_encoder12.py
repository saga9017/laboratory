...
def generate_batch(MAX_TOKEN):
    ...
    label_num = 0
    max_len = 1
    with open('cnn_data/odp.wsdm.train.test.txt', 'r') as f:
        while True:
    ...
    for i in key:
        ...
        if num_sen == max_num_sen:
            ...
            batch_dev.append((temp_X, temp_y))
            temp_X = []
            temp_y = []
    ...
    for i in key:
        ...
        if num_sen == max_num_sen:
            ...
            batch_test.append((temp_X, temp_y))
            temp_X = []
            temp_y = []
...
class Multi_head_attention(nn.Module):
    def __init__(self, hidden_dim=300, hidden_dim_=768, dropout=Dropout):
        super().__init__()
    ...
    def forward(self, en, de, mask, dropout=False):  # x : (input_len, hidden_dim)
        ...
        a = torch.cat((head3[0], head3[1], head3[2], head3[3], head3[4], head3[5], head3[6], head3[7], head3[8], head3[9], head3[10], head3[11]), 2)
...
class Encoder(nn.Module):
    def __init__(self ,hidden_dim, hidden_dim_):  # default=512
        ...
        self.encoders=nn.ModuleList(Encoder_layer(hidden_dim, hidden_dim_) for _ in range(12))

    def input_embedding(self, x):  # x: (batch, input_len, )
        mask_e=Parameter((x==0).unsqueeze(1).repeat(1,x.shape[1], 1), requires_grad=False)
        return self.embedded_matrix[x]+self.sequence_embed[:x.shape[1]].unsqueeze(0).repeat(x.shape[0], 1, 1), mask_e # (input_len, hidden_dim)
    ...
    def forward(self, x, dropout):
        ...
        for encoder in self.encoders:
            result=encoder(result, mask_e, non_pad_mask, dropout=dropout)
...
class bert(nn.Module):
    ...
    def predict(self, x):
        ...
        ###############################################################################################################
        x=self.encoder(x, dropout=False)
        ###############################################################################################################
        x = self.softmax(self.V_d(x[:,0]))
    ...
    def epoch_eval(self, batch_dev):
        ...
        for i in range(len(batch_dev)):
            prediction=self.predict(torch.tensor(batch_dev[i][0]).to(device)).cpu()
            flags = ( prediction== torch.tensor(batch_dev[i][1]))
            num_flags+=len(flags)
            score += torch.sum(flags).item()
            f1_score=compute_metrics(prediction, torch.tensor(batch_dev[i][1]), 2531)
    ...
    def train_with_batch(self, batch, batch_dev, batch_test, evaluate_loss_after=5):
        ...
        for epoch in range(nepoch):
            ...
            torch.save(model.state_dict(), 'transformer_encoder12_saved_model/checkpoint'+str(epoch))
            print('saved')
        return last_loss

torch.manual_seed(10)
batch, key_dic, y_number, batch_dev, batch_test, label_dic= generate_batch(MAX_TOKEN)
torch.save(label_dic, 'transformer_encoer12_label_dic')
torch.save(key_dic, 'transformer_encoer12_key_dic')
torch.save(y_number, 'transformer_encoer12_y_number')
# Train on a small subset of the data to see what happens
model = bert(y_number).to(device)
