...
두번째 방법은 log를 씌어 반올림한 수치를 범주로 보는 것입니다. 예를 들어, 예수금이 1억이면 log(100000000)를 반올림한 수치를 사용하는 것이죠.
"""""""""""""""""""""""""""""""""""""""""
        ...
        for index, data in enumerate(row):
            if index in used_index:
                if index==3:
                    ...
                    temp.append(rounding(math.log(data.value)))
                elif index==11:
                    if data.value=='-':
                        temp.append(0)
                    else:
...
class classification(nn.Module):
    def __init__(self):  # default=512
        ...
        self.embeddings=nn.ModuleList([nn.Embedding(2, 128), nn.Embedding(9, 128), nn.Embedding(2, 128), nn.Embedding(21421, 128),
                                       nn.Embedding(22, 128), nn.Embedding(22, 128), nn.Embedding(22, 128), nn.Embedding(22, 128)])
