"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, freeze, using text, trained image feature + location + knowledge
...
class Transformer_new(nn.Module):
    ...
    def input_embedding(self, batch_text,  batch_img, batch_know):  # x: (batch, input_len, )

        b_size, s_len, _=batch_text.shape
        mixed=torch.cat([batch_img, self.know_embed(batch_know), batch_text], dim=1)
        segment=torch.tensor([0]*49+[1]*6+[2]*s_len).to(device)
    ...
    def forward(self, batch_text, batch_img, batch_know, attention_mask, dropout):
        b_size, s_len, _=batch_text.shape
        x1= self.input_embedding(batch_text, batch_img, batch_know)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        if device.type=='cpu':
...
class BERTLM_new(nn.Module):
    ...
    def forward(self, batch_loc_text, batch_img, batch_know, segment, dropout):
        ...
        x=self.transformer_new(x, batch_img, batch_know, attention_mask, dropout)
    ...
    def final_test(self, batch_test):
        test_precision = []
        ...
        for i in range(len(batch_test)):
            model.eval()
            batch_loc_text, batch_img, batch_know, segment, batch_y, true_label = batch_test[i]
            test_prob = self.softmax(
            ...
            y_pred = np.argsort(test_prob.detach().cpu().numpy(), axis=1)
            for i in range(y_pred.shape[0]):
                test_pred.append(y_pred[i])
                test_truth.append(true_label[i])
        precision = top1_acc(test_truth, test_pred)
...
if __name__ == '__main__':
    ...
    for index, sentence in enumerate(val_data[2][:, -17:]):
        ...
        temp_sen=tokenizer.encode(' '.join([vocabulary_inv[j] for j in temp_sen]))
    ...
    checkpoint = torch.load('project3_bert20_2_checkpoint/epoch=2')  #수정 필요!!!
    model.load_state_dict(checkpoint['model_state_dict'])

    print("starts training...")

    test_precision = model.final_test(batch_test)

    print("\ntop1 결과 정리")
    print("test")
    for i in range(len(test_precision)):
        print("epoch", i + 1, "- precision:", test_precision[i])
