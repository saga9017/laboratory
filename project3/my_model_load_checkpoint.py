"""""""""""""""""""""""""""
batch suffle, encoder 6, fixed learning rate, reduce hidden_dim, not using word2vec
...
class Multi_head_attention(nn.Module):
    def __init__(self, hidden_dim=300, hidden_dim_=512, dropout=Dropout):
        ...
        self.layerNorm_add_Norm = nn.LayerNorm(hidden_dim)
...
class Bert(nn.Module):
    def __init__(self, word_dim1, hidden_dim=hidden_dim, hidden_dim_=hidden_dim):  # default=512
        ...
        #self.token_embed=nn.Embedding.from_pretrained(torch.from_numpy(embedding_matrix).float(), freeze=False)
        self.token_embed = nn.Embedding(word_dim1, hidden_dim)
        self.know_embed=nn.Embedding(word_dim1, hidden_dim)

        """""
        self.linear_for_token = nn.Linear(300, hidden_dim)
        self.linear_for_img=nn.Linear(512, hidden_dim)

        nn.init.xavier_normal_(self.linear_for_token.weight)
        nn.init.xavier_normal_(self.linear_for_img.weight)
        """""

        self.segment_embed = nn.Embedding(4, hidden_dim)
        self.sequence_embed = Parameter(torch.randn(text_maxlen+2+(49+1)+(17+1)+(5+1), hidden_dim))
    ...
    def input_embedding(self, batch_text, batch_img, batch_loc, batch_know, segment):  # x: (batch, input_len, )
        """""
        ...
        """""
        b_size=batch_text.shape[0]
        new_batch_loc=self.token_embed(batch_loc)
        new_batch_img = batch_img.view(b_size, 512, 49).transpose(1, 2)
        new_batch_text=self.token_embed(batch_text)
        mixed=torch.cat([new_batch_loc, self.know_embed(batch_know), new_batch_img, new_batch_text], dim=1)
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x1= self.input_embedding(batch_text, batch_img, batch_loc, batch_know, segment)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        if device.type=='cpu':
        ...
        non_pad_mask = torch.cat([margin, (batch_text != 0)], dim=1).unsqueeze(-1).repeat(1, 1, self.hidden).float()
        mask_e = (torch.cat([margin, (batch_text != 0)], dim=1).unsqueeze(1).repeat(1, x1.shape[1], 1)==0)
...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        x = self.bert(batch_text, batch_img, batch_loc, batch_know, segment, dropout)
    ...
    def train_with_batch(self, info, batch_val, optimizer):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                lrate = math.pow(64, -0.5) * min(math.pow(num_examples_seen, -0.5), num_examples_seen * math.pow(4000, -1.5))  # warm up step : default 4000
                loss= self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, segment, batch_y), optimizer, lrate, True)
                self.Loss+=loss.item()
                Loss_len += 1
            ...
            for i in range(len(batch_val)):
                ...
                val_prob = self.softmax(self.forward(torch.tensor(batch_text).to(device).long(), torch.tensor(batch_img).to(device).float(),
                         torch.tensor(batch_loc).to(device).long(), torch.tensor(batch_know).to(device).long(), torch.tensor(segment).to(device).long(),False))
            ...
            }, 'project3_6_checkpoint/epoch='+str(epoch))
...
def generate_batch(new_text_list, data, len_hashtag):
    ...
    for i in key:
        ...
        if len(i[1:]) <= k:
            temp_text.append(i[1:] + [0] * (k - len(i[1:])))
        else:
            print('error')
            temp_text.append(i[1:])
...
if __name__ == '__main__':
    ...
    """
    wv_model = gensim.models.KeyedVectors.load_word2vec_format('project3_data/GoogleNews-vectors-negative300.bin.gz',
                                                            binary=True)

    embedding_matrix = np.zeros((len(vocabulary), 300))
    for key in vocabulary.keys():
        if key not in wv_model.wv:
            embedding_vector = np.random.uniform(-0.001, 0.001, 300)
        else:
            embedding_vector = wv_model.wv[key]
        embedding_matrix[vocabulary[key]] = embedding_vector
    ...
    checkpoint = torch.load('project3_6_checkpoint/epoch='+str(131))
    ...
    val_precision = model.train_with_batch((new_train_text_list, train_data, len(hashtagVoc)), batch_val, optimizer)
