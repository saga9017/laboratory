"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, fine-tuning, using text, trained image feature + location + knowledge+ torch transformer
...
class Transformer_new(nn.Module):
    def __init__(self, hidden_dim=hidden_dim, hidden_dim_=hidden_dim):  # default=512
        ...
        self.encoder = nn.TransformerEncoder(encoder_layers, 3)
    ...
    def forward(self, batch_text, batch_img, batch_know, attention_mask, dropout):
        ...
        src_padding_mask=torch.cat([margin, attention_mask], dim=1).byte()
        src_padding_mask = torch.BoolTensor(src_padding_mask)
        ################################################################################################################
        x1=x1.masked_fill(non_pad_mask==0, 0)
        ########################################################
        x2=self.encoder(x1)#, src_key_padding_mask=src_padding_mask)
