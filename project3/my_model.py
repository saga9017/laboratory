...
class Bert(nn.Module):
    def __init__(self, word_dim1, hidden_dim=hidden_dim, hidden_dim_=hidden_dim):  # default=512
        ...
        self.loc_embed=nn.Embedding(word_dim1, hidden_dim)
        self.know_embed=nn.Embedding(word_dim1, hidden_dim)

        self.seg=torch.tensor([[0]*text_maxlen+[1]*(49+1)+[2]*(17+1)+[3]*(5+1)]).to(device)
        self.segment_embed = nn.Embedding(4, hidden_dim)
        self.sequence_embed = Parameter(torch.randn(text_maxlen+(49+1)+(17+1)+(5+1), hidden_dim))

        #weight initialization
        self.token_embed.weight.data.uniform_(-0.01, 0.01)
        self.loc_embed.weight.data.uniform_(-0.01, 0.01)
    ...
    def input_embedding(self, batch_text, batch_img, batch_loc, batch_know):  # x: (batch, input_len, )
        """""
        print('text :', self.token_embed(batch_text).shape)
        print('img :', batch_img.view(batch_size(), 512, 49). transpose(1,2).shape)
        print('loc :', self.loc_embed(batch_loc[:, -9:]).shape)
        print('know :', self.know_embed(batch_know).shape)
        """""
        mixed=torch.cat([self.token_embed(batch_text), batch_img.view(batch_size(), 512, 49).transpose(1,2),
                         self.loc_embed(batch_loc[:, -19:]), self.know_embed(batch_know)], dim=1)
        return mixed+self.segment_embed(self.seg)+self.sequence_embed.unsqueeze(0).repeat(batch_text.shape[0], 1, 1)  # (input_len, hidden_dim)

    def forward(self, batch_text, batch_img, batch_loc, batch_know, dropout):
        b_size, s_len=batch_text.shape
        x1= self.input_embedding(batch_text, batch_img, batch_loc, batch_know,)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        if device.type=='cpu':
        ...
        non_pad_mask = torch.cat([(batch_text != 0), margin], dim=1).unsqueeze(-1).repeat(1, 1, self.hidden).float()
        mask_e = torch.cat([(batch_text != 0), margin], dim=1).unsqueeze(1).repeat(1, x1.shape[1], 1)
...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know):
        x = self.bert(batch_text, batch_img, batch_loc, batch_know, 0.1)
        return self.next_sentence(x)

    def bptt(self, batch_text, batch_img, batch_loc, batch_know, label):  # (batch_size, out_len)
        next_sentence = self.forward(batch_text, batch_img, batch_loc, batch_know)

        # 2-1. NLL(negative log likelihood) loss of is_next classification result
        next_loss = self.criterion_next_sen(next_sentence, label.squeeze(-1))
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate):
        ...
        loss = self.bptt(torch.tensor(batch[0]).to(device).long(), torch.tensor(batch[1]).to(device).float(),
                         ...
                         torch.tensor(batch[4]).to(device).long())
        loss.backward()
        optimizer.step()
    ...
    def train_with_batch(self):
        ...
        optimizer = torch.optim.Adam(model.parameters(), betas=(0.9, 0.999), eps=1e-9, lr=0.001)
        Loss_len = 0
        num_examples_seen = 1
        nepoch=nb_epoch()
        batch_len=len(train_data[3]) * 22.5 / batch_size()    # 22.5 = average hashtag number
        for epoch in range(nepoch):
            ...
            while True:
                ...
                loss= self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, batch_y), optimizer, lrate)
                self.Loss+=loss.item()
                Loss_len += 1

                if num_examples_seen % 1000 == 0:   # origin = int(batch_len * nepoch / 100)
                    time_ = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    print(time_, ' ', int(100 * num_examples_seen / (batch_len * nepoch)), end='')
                    print('%   완료!!!', end='')
            ...
            print('Epoch', epoch + 1, 'completed out of', nb_epoch())
            # valid set : 8104개
            val_pnt = 0
            y_cnt = 0
            val_pred = []
            ...
            while True:
                ...
                val_prob = self.forward(torch.tensor(batch_text).to(device).long(), torch.tensor(batch_img).to(device).float(),
            ...
            }, 'project3_checkpoint/epoch='+str(epoch))
...
if __name__ == '__main__':
    ...
    new_val_data1=np.zeros((val_data[1].shape[0], val_data[1].shape[1]+2))
    for index, sentece in enumerate(val_data[1]):
        ...
        temp_sen.append(2)
        new_val_data1[index]=temp_sen
    val_data[1]=new_val_data1
    ...
    new_test_data1 = np.zeros((test_data[1].shape[0], test_data[1].shape[1] + 2))
    for index, sentece in enumerate(test_data[1]):
        ...
        for word in sentece:
            if word==0:
                ...
                temp_sen.append(word+2)
        temp_sen.append(2)
        new_test_data1[index]=temp_sen
    test_data[1] = new_test_data1
    ...
    new_train_data1 = np.zeros((train_data[1].shape[0], train_data[1].shape[1] + 2))
    for index, sentece in enumerate(train_data[1]):
        ...
        for word in sentece:
            if word==0:
                ...
                temp_sen.append(word+2)
        temp_sen.append(2)
        new_train_data1[index]=temp_sen
    train_data[1] = new_train_data1
    ...
    text_maxlen = len(train_data[1][0])  # 413 맞는지 확인
    ...
    val_precision = model.train_with_batch()
