"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, fine-tuning
...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x=self.transformer(x, new_batch_img, attention_mask, dropout)
    ...
    def numpy_sdg_step(self, batch, optimizer1, optimizer2, lrate, dropout):
    ...
    def train_with_batch(self, info, batch_val):
        ...
        optimizer1 = torch.optim.Adam(self.bert.parameters(), betas=(0.9, 0.999), eps=1e-9, lr=1e-5)
        optimizer2 = torch.optim.Adam(list(self.next_sentence.parameters())+list(self.linear_for_img.parameters())+list(self.transformer.parameters()) , betas=(0.9, 0.999), eps=1e-9, lr=1e-3)
        Loss_len = 0
        num_examples_seen = 1
        # nepoch=nb_epoch()
        nepoch = 1000
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                loss = self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, segment, batch_y), optimizer1, optimizer2,
                                           lrate, True)
                self.Loss += loss.item()
                Loss_len += 1
            ...
            }, 'project3_bert6_checkpoint/epoch='+str(epoch))
