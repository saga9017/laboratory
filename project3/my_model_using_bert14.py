"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, using saved text, trained image by transformer
...
class Transformer_new(nn.Module):
    def __init__(self, hidden_dim=hidden_dim, hidden_dim_=hidden_dim):  # default=512
        ...
        self.segment_embed = nn.Embedding(2, hidden_dim)
        self.sequence_embed = Parameter(torch.randn(512+(49+1), hidden_dim))
    ...
    def input_embedding(self, batch_text,  batch_img):  # x: (batch, input_len, )

        b_size, s_len, _=batch_text.shape
        mixed=torch.cat([batch_img, batch_text], dim=1)
        segment=torch.tensor([0]*49+[1]*s_len).to(device)
    ...
    def forward(self, batch_text, batch_img, attention_mask, dropout):
        b_size, s_len, _=batch_text.shape
        x1= self.input_embedding(batch_text, batch_img)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        if device.type=='cpu':
...
class BERTLM_new(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, batch_mask, dropout):
        attention_mask=batch_mask
        x=self.transformer_new(batch_text, batch_img, attention_mask, dropout)
        return self.next_sentence(x)

    def bptt(self, batch_text, batch_img, batch_loc, batch_know, segment, label, batch_mask, dropout):  # (batch_size, out_len)
        next_sentence = self.forward(batch_text, batch_img, batch_loc, batch_know, segment, batch_mask, dropout)
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate, dropout):
        ...
        optimizer.zero_grad()
        loss = self.bptt(torch.tensor(batch[0]).to(device).float(), torch.tensor(batch[1]).to(device).float(),
                         torch.tensor(batch[2]).to(device).long(), torch.tensor(batch[3]).to(device).long(),
                         torch.tensor(batch[4]).to(device).long(), torch.tensor(batch[5]).to(device).float(),
                         torch.tensor(batch[6]).to(device).bool(), dropout)
...
def generate_batch_train(new_text_list, data, len_hashtag):
    batch = []
    ...
    for i in key:
        ...
        temp_text.append(i[0])
        temp_mask.append(data[6][i[0]])
        ...
        if num_sen == max_num_sen:
            ...
            segment = [0] * len(temp_loc[0]) + [1] * len(temp_know[0]) + [2] * (49 + 1) + [3] * (len(temp_text) - 1)
            batch.append((temp_text, temp_img, temp_loc, temp_know, segment, temp_y, temp_y_, temp_mask))
            temp_text = []
            temp_mask = []
            temp_img = []
            temp_loc = []
...
if __name__ == '__main__':
    ...
    val_data = []
    val_data.append(np.load("project3_transformer_data/transformer_image_71_val.npy"))
    ...
    val_data.append(np.load("project3_transformer_data/bert_text_val.npy"))
    val_data.append(np.load("project3_transformer_data/bert_mask_val.npy"))

    print(len(val_data[0]), len(val_data[1]), len(val_data[2]), len(val_data[3]), len(val_data[4]), len(val_data[5]), len(val_data[6]))
    # val_data = check_hashzero(val_data)
    # print("check 완")

    test_data = []
    test_data.append(np.load("project3_transformer_data/transformer_image_71_test.npy"))
    ...
    test_data.append(np.load("project3_transformer_data/bert_text_test.npy"))
    test_data.append(np.load("project3_transformer_data/bert_mask_test.npy"))

    print(len(test_data[0]), len(test_data[1]), len(test_data[2]), len(test_data[3]), len(test_data[4]), len(test_data[5]), len(test_data[6]))
    ...
    train_image.append(np.load("project3_transformer_data/transformer_image_71_train1.npy"))
    train_image.append(np.load("project3_transformer_data/transformer_image_71_train2.npy"))
    train_image.append(np.load("project3_transformer_data/transformer_image_71_train3.npy"))
    train_image.append(np.load("project3_transformer_data/transformer_image_71_train4.npy"))
    ...
    train_mask.append(np.load("project3_transformer_data/bert_mask_train1.npy"))
    train_mask.append(np.load("project3_transformer_data/bert_mask_train2.npy"))
    train_mask.append(np.load("project3_transformer_data/bert_mask_train3.npy"))
    train_mask.append(np.load("project3_transformer_data/bert_mask_train4.npy"))
    train_mask = np.concatenate(train_mask, axis=0)
    train_data.append(train_mask)

    print(len(train_data[0]), len(train_data[1]), len(train_data[2]), len(train_data[3]), len(train_data[4]), len(train_data[5]), len(train_data[6]))
