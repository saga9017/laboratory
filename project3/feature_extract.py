"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, freeze, using text, trained image by transformer
...
class Transformer(nn.Module):
    ...
    def input_embedding(self, batch_img):  # x: (batch, input_len, )

        b_size, s_len, _=batch_img.shape
        #mixed=torch.cat([batch_img, batch_text], dim=1)
        mixed=batch_img
        segment=torch.tensor([0]*49).to(device)
    ...
    def forward(self, batch_img, dropout):
        b_size, s_len, _=batch_img.shape
        x1= self.input_embedding(batch_img)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        non_pad_mask = torch.ones((b_size, 49)).to(device).unsqueeze(-1).repeat(1, 1, self.hidden).float()
        mask_e = (torch.ones((b_size, 49)).to(device).unsqueeze(1).repeat(1, x1.shape[1], 1)==0)
    ...
    def forward2(self, batch_img, dropout):
        b_size, s_len, _=batch_img.shape
        x1= self.input_embedding(batch_img)  # (input_len, hidden)
        ###################################### make non_pad_mask   #####################################################
        non_pad_mask = torch.ones((b_size, 49)).to(device).unsqueeze(-1).repeat(1, 1, self.hidden).float()
...
class BERTLM(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        self.linear_for_img = nn.Linear(512, 768)
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        new_batch_img = self.linear_for_img(batch_img.view(b_size, 512, 49).transpose(1, 2))
        x=self.transformer.forward2(new_batch_img, dropout)
        return x
...
class BERTLM_new(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        self.bert = Bert()
        self.next_sentence = NextSentencePrediction(768)
        self.bertlm=BERTLM(vocab_size).to(device)
        checkpoint = torch.load('project3_bert10-2_checkpoint/epoch=' + str(383))
        self.bertlm.load_state_dict(checkpoint['model_state_dict'])
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x = self.bert(batch_text, attention_mask=attention_mask.float())
        new_batch_img = self.bertlm(batch_text, batch_img, batch_loc, batch_know, segment, dropout)
        return x, attention_mask, new_batch_img

    def bptt(self, batch_text, batch_img, batch_loc, batch_know, segment, label, dropout):  # (batch_size, out_len)
        x, attention_mask, new_batch_img = self.forward(batch_text, batch_img, batch_loc, batch_know, segment, dropout)
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate, dropout):
        ...
        x, attention_mask, new_batch_img = self.bptt(torch.tensor(batch[0]).to(device).long(), torch.tensor(batch[1]).to(device).float(),
    ...
    def train_with_batch(self, info):
        list_x=[]
        list_attention_mask=[]
        list_new_batch_img=[]
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                x, attention_mask, new_batch_img = self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, segment, batch_y), optimizer,
                                           lrate, True)


                list_x.append(x.cpu().detach().numpy())
                list_attention_mask.append(attention_mask.cpu().detach().numpy())
                list_new_batch_img.append(new_batch_img.cpu().detach().numpy())

                if num_examples_seen % 1000==0:
        ...
        text=np.concatenate(list_x, axis=0)
        mask=np.concatenate(list_attention_mask, axis=0)
        image=np.concatenate(list_new_batch_img, axis=0)

        np.save('project3_transformer_data/bert_text_val', text)
        np.save('project3_transformer_data/bert_mask_val', mask)
        np.save('project3_transformer_data/transformer_image_71_val', image)
        return text, mask, image
...
if __name__ == '__main__':
    ...
    text, mask, image = model.train_with_batch((new_train_text_list, train_data, len(hashtagVoc)))
