"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction + last linear
...
class Last_layer(nn.Module):
    def __init__(self):
        ...
        super().__init__()
        self.linear=nn.Linear(768, hashtag_size)

    def forward(self, x, new_batch_img, attention_mask, dropout):
        output=torch.cat([new_batch_img, x], dim=1)
        output=self.linear(torch.mean(output, dim=1))
        return output
...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x=self.last_layer(x, new_batch_img, attention_mask, dropout)
        return x
