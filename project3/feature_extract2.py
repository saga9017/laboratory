"""""""""""""""""""""""""""
using cls for prediction, freeze, only using pre-trainedimage + transformer6
...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x=self.transformer.forward2(batch_img, dropout)
        return x

    def bptt(self, batch_text, batch_img, batch_loc, batch_know, segment, label, dropout):  # (batch_size, out_len)
        new_batch_img = self.forward(batch_text, batch_img, batch_loc, batch_know, segment, dropout)
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate, dropout):
        ...
        new_batch_img= self.bptt(torch.tensor(batch[0]).to(device).long(), torch.tensor(batch[1]).to(device).float(),
    ...
    def train_with_batch(self, info, batch_val):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                new_batch_img = self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, segment, batch_y), optimizer,
                                           lrate, True)
        ...
        print(image.shape)
        np.save('project3_transformer_data/transformer_image_90_train', image)
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('project3_bert13_checkpoint/epoch=' + str(180))
