...
class BERTLM_new(nn.Module):
    ...
    def forward(self, batch_loc_text, batch_img, batch_know, segment, dropout):
        ...
        if random.random()<0.2:
            loc_len=torch.sum(segment[0]==1)
            batch_loc_text=batch_loc_text[:, loc_len:]
            segment=segment[:,loc_len:]
            attention_mask=attention_mask[:, loc_len:]
    ...
    def train_with_batch(self, info, batch_val, optimizer):
        ...
        for epoch in range(nepoch):
            ...
            }, 'project3_partial1_checkpoint/epoch='+str(epoch))
