...
class Bert(nn.Module):
    ...
    def input_embedding(self, batch_text, batch_img, batch_loc, batch_know, segment):  # x: (batch, input_len, )
        ...
        mixed=torch.cat([self.token_embed(batch_loc), self.know_embed(batch_know), batch_img.view(b_size, 512, 49).transpose(1,2),
                         self.token_embed(batch_text)], dim=1)
...
class BERTLM(nn.Module):
    ...
    def train_with_batch(self, batch_train, batch_val):
        val_precision = []
        ...
        print('lengh of batch :', len(batch_train))
        for epoch in range(nepoch):
            ...
            }, 'project3_2_checkpoint/epoch='+str(epoch))
...
def generate_batch(new_text_list, data, len_hashtag):
    ...
    key = list(reversed(sorted(new_text_list, key=len)))

    key_dic = {}

    k = len(key[0][1:])  # 현재 가장 긴 문장의 길이

    if int(MAX_TOKEN / k) == 0:
        max_num_sen = 1
    else:
        max_num_sen = int(MAX_TOKEN / k)  # 한 batch 안에 저장할 token의 개수
...
if __name__ == '__main__':
    ...
    train_data.append(np.load("./project3_data/val_img_x_c_h.npz")["img"])
    ...
    batch_train = generate_batch(new_train_text_list[:5], train_data, len(hashtagVoc))
    batch_val = generate_batch(new_val_text_list[:5], val_data, len(hashtagVoc))
    batch_test = generate_batch(new_test_text_list[:5], test_data, len(hashtagVoc))
    print(batch_train)
    ...
    val_precision = model.train_with_batch(batch_train, batch_train)
