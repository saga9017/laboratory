...
class BERTLM(nn.Module):
    ...
    def forward(self, batch_text, batch_img, batch_loc, batch_know, segment, dropout):
        ...
        x=self.transformer(batch_img, dropout)
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate, dropout):
        ...
        optimizer.zero_grad()
        loss, sigmoid = self.bptt(torch.tensor(batch[0]).to(device).long(), torch.tensor(batch[1]).to(device).float(),
    ...
    def train_with_batch(self, info, batch_val):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                loss, train_prob = self.numpy_sdg_step((batch_text, batch_img, batch_loc, batch_know, segment, batch_y), optimizer,
                                           lrate, True)
                self.Loss += loss.item()
                Loss_len += 1

                y_pred = np.argsort(train_prob.detach().cpu().numpy(), axis=1)
                for i in range(y_pred.shape[0]):
                    train_pred.append(y_pred[i])
                    train_truth.append(true_label[i])
                ...
                num_examples_seen += 1
            precision_train = top1_acc(train_truth, train_pred)
            ...
            print("Epoch:", (epoch + 1), "train_precision:", precision_train, "val_precision:", precision)
            ...
            }, 'project3_bert13_checkpoint/epoch='+str(epoch))
