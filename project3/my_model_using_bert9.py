"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, freeze, only using text
...
class Transformer(nn.Module):
    ...
    def input_embedding(self, batch_text,  batch_img):  # x: (batch, input_len, )
        ...
        mixed = batch_text
        segment=torch.tensor([1]*s_len).to(device)
    ...
    def forward(self, batch_text, batch_img, attention_mask, dropout):
        ...
        ###################################### make non_pad_mask   #####################################################
        non_pad_mask = attention_mask.unsqueeze(-1).repeat(1, 1, self.hidden).float()
        mask_e = (attention_mask.unsqueeze(1).repeat(1, x1.shape[1], 1)==0)
...
class BERTLM(nn.Module):
    ...
    def train_with_batch(self, info, batch_val):
        ...
        for epoch in range(nepoch):
            ...
            }, 'project3_bert9_checkpoint/epoch='+str(epoch))
