...
for _ in range(epoch):
    for i in range(len(label)):
        ...
        output_label = int(label[i])-1

        mean_w_in=np.mean(w_in[input_word_numbers], axis=0)

        y = softmax(np.dot(w_out.T, mean_w_in))
        ...
        #w_out을 먼저 update 한다.
        w_out-= alpha * np.dot(mean_w_in.reshape(-1,1) , e.T)
