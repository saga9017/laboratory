"""""""""""""""""""""""""""
using pre-trained bert, using cls for prediction, fine-tuning, only using text
...
class Norm(nn.Module):
    ...
    def forward(self, X):
        ...
        X_norm = torch.div(X - mu.view(X.shape[0], X.shape[1], 1),
                           torch.sqrt(var.view(X.shape[0], X.shape[1], 1) + 1e-8))
...
class Transformer_new(nn.Module):
    ...
    def input_embedding(self, batch_text):  # x: (batch, input_len, )
        ...
        return batch_text + self.segment_embed(segment) + self.sequence_embed[:batch_text.shape[1]].unsqueeze(0).repeat(
            b_size, 1, 1)  # (input_len, hidden_dim)
...
class BERTLM_new(nn.Module):
    ...
    def bptt(self, batch_loc_text, batch_img, batch_know, segment, label, dropout):  # (batch_size, out_len)
        ...
        loss = self.criteria(next_sentence, label)
        """""
    ...
    def numpy_sdg_step(self, batch, optimizer, lrate, dropout):
        ...
        loss = self.bptt(torch.tensor(batch[0]).to(device).long(), torch.tensor(batch[1]).to(device).float(),
                         ...
                         torch.tensor(batch[4]).to(device).long(), dropout)
    ...
    def train_with_batch(self, info, batch_val, optimizer):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch_train)):
                ...
                loss = self.numpy_sdg_step((batch_loc_text, batch_img, batch_know, segment, true_label), optimizer,
                                           lrate, True)
                self.Loss += loss.item()
                Loss_len += 1

                if num_examples_seen % 400 == 0:  # origin = int(batch_len * nepoch / 100)
            ...
            for i in range(len(batch_val)):
                ...
                val_prob = self.softmax(
                    self.forward(torch.tensor(batch_loc_text).to(device).long(),
                                 torch.tensor(batch_img).to(device).float(),
                                 torch.tensor(batch_know).to(device).long(), torch.tensor(segment).to(device).long(),
                                 False))
...
if __name__ == '__main__':
    ...
    with open('new_test.pickle', 'rb') as f:
    ...
    for idx, i in enumerate(val_data_text):
        new_val_loc_text_list.append([idx] + tokenizer.encode(i[1]) + tokenizer.encode(i[0])[1:])
        new_val_loc_list.append([idx] + tokenizer.encode(i[1])[:-1])
        val_label.append(i[2])
    ...
    ############################################################################
    batch_val = generate_batch(new_val_loc_text_list, new_val_loc_list, val_data, hashtag_size)
    ############################################################################
    ...
    for idx, i in enumerate(train_data_text):
        new_train_loc_text_list.append([idx] + tokenizer.encode(i[1]) + tokenizer.encode(i[0])[1:])
        new_train_loc_list.append([idx] + tokenizer.encode(i[1])[:-1])
    ...
    val_precision = model.train_with_batch((new_train_loc_text_list, new_train_loc_list, train_data, hashtag_size),
                                           batch_val, optimizer)
