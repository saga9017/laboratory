...
SST_1_dev=[]
SST_1_test=[]
with open('CNN Data/SST-1/stsa.fine.train', 'r', encoding='latin-1') as f:
    for row in f.readlines():
        SST_1_train.append([clean_str_sst(row[1:]), int(row[0])])

# sentences of validation data
with open('CNN Data/SST-1/stsa.fine.dev', 'r', encoding='latin-1') as f:
    for row in f.readlines():
        SST_1_dev.append([clean_str_sst(row[1:]), int(row[0])])

# sentences of test data
with open('CNN Data/SST-1/stsa.fine.test', 'r', encoding='latin-1') as f:
    for row in f.readlines():
        SST_1_test.append([clean_str_sst(row[1:]), int(row[0])])


def onehot(index):
    output=[0]*index+[1]+[0]*(5-index-1)
...
W4 = tf.get_variable("W4", shape=[100*3, 5],
                     initializer=tf.contrib.layers.xavier_initializer())
...
# Final FC 7x7x64 inputs -> 10 outputs
W4_ = tf.get_variable("W4_", shape=[100*3, 5],
                     initializer=tf.contrib.layers.xavier_initializer())
