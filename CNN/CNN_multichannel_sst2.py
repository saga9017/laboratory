...
SST_1_dev=[]
SST_1_test=[]
with open('CNN Data/SST-2/stsa.binary.train', 'r', encoding='latin-1') as f:
    for row in f.readlines():
        SST_1_train.append([clean_str_sst(row[1:]), int(row[0])])

# sentences of validation data
with open('CNN Data/SST-2/stsa.binary.dev', 'r', encoding='latin-1') as f:
    for row in f.readlines():
        SST_1_dev.append([clean_str_sst(row[1:]), int(row[0])])

# sentences of test data
with open('CNN Data/SST-2/stsa.binary.test', 'r', encoding='latin-1') as f:
    for row in f.readlines():
...
embedding_matrix_for_non_static=tf.Variable(embedding_matrix, dtype=tf.float32)

print('embedding_matrix :', embedding_matrix_for_non_static.shape)


# input place holders
X = tf.placeholder(tf.int32, [None, max_len])
X_float=tf.nn.embedding_lookup(embedding_matrix_for_non_static, X)
...
################################################################################################3
embedding_matrix_for_static=tf.constant(embedding_matrix, dtype=tf.float32)

X_float_=tf.nn.embedding_lookup(embedding_matrix_for_static, X)

X_img_ = tf.reshape(X_float_, [-1, max_len, embedding_size, 1])   # img 28x28x1 (black/white)

# L1 ImgIn shape=(?, 53, 300, 1)
W1_ = tf.get_variable("W1_", shape=[3, embedding_size, 1, 100],
                     initializer=tf.contrib.layers.xavier_initializer())
#    Conv     -> (?, 51, 1, 100)
#    Pool     -> (?, 1, 1, 100)
L1_1_ = tf.nn.conv2d(X_img_, W1_, strides=[1, 1, 1, 1], padding='VALID')
L1_2_ = tf.nn.relu(L1_1_)
L1_3_ = tf.nn.max_pool(L1_2_, ksize=[1, max_len-2, 1, 1],
                    strides=[1, max_len-2, 1, 1], padding='VALID')
L1_4_ = tf.nn.dropout(L1_3_, keep_prob)

L1_flat_ = tf.reshape(L1_4_, [-1, 100])

# L2 ImgIn shape=(?, 53, 300, 1)
W2_ = tf.get_variable("W2_", shape=[4, embedding_size, 1, 100],
                     initializer=tf.contrib.layers.xavier_initializer())
#    Conv     -> (?, 50, 1, 100)
#    Pool     -> (?, 1, 1, 100)
L2_1_ = tf.nn.conv2d(X_img_, W2_, strides=[1, 1, 1, 1], padding='VALID')
L2_2_ = tf.nn.relu(L2_1_)
L2_3_ = tf.nn.max_pool(L2_2_, ksize=[1, max_len-3, 1, 1],
                    strides=[1, max_len-3, 1, 1], padding='VALID')
L2_4_ = tf.nn.dropout(L2_3_, keep_prob)

L2_flat_ = tf.reshape(L2_4_, [-1, 100])


# L3 ImgIn shape=(?, 53, 300, 1)
W3_ = tf.get_variable("W3_", shape=[5, embedding_size, 1, 100],
                     initializer=tf.contrib.layers.xavier_initializer())
#    Conv     -> (?, 49, 1, 100)
#    Pool     -> (?, 1, 1, 100)
L3_1_ = tf.nn.conv2d(X_img_, W3_, strides=[1, 1, 1, 1], padding='VALID')
L3_2_ = tf.nn.relu(L3_1_)
L3_3_ = tf.nn.max_pool(L3_2_, ksize=[1, max_len-4, 1, 1],
                    strides=[1, max_len-4, 1, 1], padding='VALID')
L3_4_ = tf.nn.dropout(L3_3_, keep_prob)

L3_flat_ = tf.reshape(L3_4_, [-1, 100])



# Final FC 7x7x64 inputs -> 10 outputs
W4_ = tf.get_variable("W4_", shape=[100*3, 2],
                     initializer=tf.contrib.layers.xavier_initializer())
b_ = tf.Variable(tf.random_normal([2]))
logits_ = tf.matmul(tf.concat([L1_flat_ ,L2_flat_ ,L3_flat_], 1), W4_) + b_

W5 = tf.get_variable("W5", shape=[2*2, 2],
                     initializer=tf.contrib.layers.xavier_initializer())


# define cost/loss & optimizer
#cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=tf.reduce_sum([logits, logits_], 0), labels=Y))
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=tf.matmul(tf.concat([logits, logits_], 1), W5), labels=Y))
