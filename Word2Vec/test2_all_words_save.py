...
for word in words:
    if word != '':
        prime_word_of_index[len(prime_word_of_index)]=word
        if word in prime_dic.keys():
            prime_dic[word] += 1
        else:
            prime_dic[word] = 1

#print('prime_word_of_index :', prime_word_of_index)
...
#원래 있던 word2vec으로 구해진 vector를 가져온다.
loaded_dic, loaded_matrix=load_wordvector.load_wordvector()
w_in=np.zeros((len(words), embeding_size) )
for i in range(len(words)):
    if prime_word_of_index[i] not in loaded_dic:
        w_in[i] = loaded_matrix[loaded_dic['UNKNOWN']]
    else:
        w_in[i]=loaded_matrix[loaded_dic[prime_word_of_index[i]]]


#w_in = np.array([[random.uniform(-1, 1) for i in range(embeding_size)] for j in range(len(words))])
...
for epoch in range(epochs):
    ...
    for i in range(len(input)):
        ...
        # 단어의 index
        input_word_number = int(input_word[-1])
...
def save_total(file_name):
    f = open(file_name, 'w')
    for i in range(len(words)):
        vector_str = ' '.join([str(s) for s in w_in[i]])
        f.write('%s %s\n' % (words[i], vector_str))
