...
subsampling_table = {x: ([0]*int(y*1000))+([1]*(1000-int(y*1000))) for x, y in subsampling_prob.items()}
...
for i in range(len(words)):
    if words[i] not in dic.keys():
        words[i] = 'UNKNOWN'
    sampling_index=random.choice(subsampling_table[words[i]])
    if sampling_index==1:
        if i < window_size:
...
save('skip-gram_hierarchical-softmax')
