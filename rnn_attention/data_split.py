...
view_sentence_len=20000       #50000 train, 5000 test
batch_size=512
unknown_number=1
MAX_LENGTH = 30
...
pairs_train=pairs[:50000]
pairs_test=pairs[50000:]
...
input_lang_sorted=sorted(input_lang.word2count.items(), key=f2)
input_lang_sorted.reverse()
print(input_lang_sorted)
frequent_list=[]
for x,y in input_lang_sorted:
    if y>400:
        frequent_list.append(x)
...
for sen1, sen2 in pairs:
    ...
    for word in sen1.split():
        if word in frequent_list:
            pass
        else:
            flag=False

    if flag==True:
        test_set.append([sen1, sen2])
        len_test+=1
    else:
        train_set.append([sen1, sen2])
