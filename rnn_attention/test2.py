...
class LSTM_attention(nn.Module):
    ...
    def numpy_sdg_step(self, x, y, learning_rate):
        ...
        for i in model.parameters():
            temp_parameters.append(torch.tensor(i))
        ...
        optimizer.step()
        """""""""""
        업데이트 된 상태에서 새로운 loss를 구한다. 새로운 loss가 이전 loss보다 클 경우 learning rate를 바꾼다.
        """""""""""
        loss2=self.bptt(x,y)
        if loss2>loss:
            #새로운 loss가 이전 loss보다 클 경우 parameters을 다시 되돌린다.
            for i, j in zip(model.parameters(), temp_parameters):
                i.data=j
            ...
            if loss3>loss:
                ...
                for i, j in zip(model.parameters(), temp_parameters):
                    i.data = j
                learning_rate= 1.1*(learning_rate/0.9)
                optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
                optimizer.zero_grad()
                loss.backward(retain_graph=True)
                optimizer.step()
    ...
    def train_with_batch(self, X_train, y_train, learning_rate=0.01, nepoch=100, evaluate_loss_after=5):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(total_step):
                # One SGD step
                if i == total_step - 1:
                    loss, learning_rate = self.numpy_sdg_step(X_train[i * batch_size:],
                                                     y_train[i * batch_size:], learning_rate)
                    self.Loss += loss
                else:
                    loss, learning_rate = self.numpy_sdg_step(X_train[i * batch_size:(i + 1) * batch_size],
