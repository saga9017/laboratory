...
class LSTM_attention(nn.Module):
    def __init__(self, word_dim1, word_dim2, hidden_dim=512):
        ...
        self.V = Parameter(torch.randn(word_dim2, 2*hidden_dim).cuda())
...
last_loss = model.train_with_batch(torch.tensor(X_train).cuda(), torch.tensor(y_train).cuda(), torch.tensor(y__train).cuda(), nepoch=10)


if len(pairs) % batch_size==0:
    total_step=int(len(pairs)/batch_size)
else:
    total_step=int(len(pairs)/batch_size)+1


predicts=[]
for i in range(total_step):
    predicts.append(model.predict(torch.tensor(X_train[i*batch_size:(i+1)*batch_size]).to(device)))
...
save('[lstm_attention]view_sentence=50000 batch_size=512 epoch=10.txt')