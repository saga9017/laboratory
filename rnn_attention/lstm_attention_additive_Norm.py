...
class RNN_attention:
    def __init__(self, word_dim1, word_dim2, hidden_dim=128):
        ...
        self.new_W1 = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (hidden_dim, hidden_dim))
        self.new_W2 = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (hidden_dim, hidden_dim))

        self.new_V = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (hidden_dim))
        ...
        self.vnew_W1 = np.zeros_like(self.new_W1)
        self.mnew_W1 = np.zeros_like(self.new_W1)

        self.vnew_W2 = np.zeros_like(self.new_W2)
        self.mnew_W2 = np.zeros_like(self.new_W2)

        self.vnew_V = np.zeros_like(self.new_V)
        self.mnew_V = np.zeros_like(self.new_V)
    ...
    def forward_propagation(self, x, y):
        ...
        for t in np.arange(T2):
            ...
            e_pre[t]=np.tanh(h[:-1].dot(self.new_W1)+np.outer(np.ones(h[:-1].shape[0]), s[t].dot(self.new_W2)))
            e=np.dot(e_pre[t], self.new_V)
    ...
    def bptt(self, x, y):
        ...
        dLdnew_V=np.zeros_like(self.new_V)
        dLdnew_W1=np.zeros_like(self.new_W1)
        dLdnew_W2=np.zeros_like(self.new_W2)
        ...
        for t in np.arange(T2)[::-1]:
            ...
            dLdnew_V+=np.dot(de, e_pre[t])
            de_pre=np.outer(de, self.new_V)
            de_pre_raw=(1 - e_pre[t] * e_pre[t]) * de_pre
            dLdnew_W1 +=np.dot(de_pre_raw.T, h[:-1])
            dLdnew_W2 +=np.dot(np.dot(np.ones(h[:-1].shape[0]), de_pre_raw), s[t])
            dh_dot_minus_one+=np.dot(de_pre_raw, self.new_W1)
            ds_2=np.dot(np.dot(np.ones(h[:-1].shape[0]), de_pre_raw), self.new_W2)
            ####################################################3
        ...
        for dparam in [dLdV , dLdU_, dLdW_, dLdnew_V, dLdnew_W1, dLdnew_W2]:
        ...
        return [dLdU, dLdW, dLdU_, dLdW_, dLdV, dLdnew_V, dLdnew_W1, dLdnew_W2, dgamma, dbeta]
    ...
    def predict(self, x):
        ...
        while z!=1 and step<=10:
            ...
            c_ = f_ * (c__pre) + i_ * (g_)
            s = o_[t] * np.tanh(c_)
            e_pre = np.tanh(h[:-1].dot(self.new_W1) + s.dot(self.new_W2))
            e = np.dot(e_pre, self.new_V)
            alpha = softmax(e)
    ...
    def numpy_sdg_step(self, x, y, learning_rate):
        ...
        dLdU, dLdW, dLdU_, dLdW_, dLdV, dLdnew_V, dLdnew_W1, dLdnew_W2, dgamma, dbeta = self.bptt(x, y)
        #print('dLdU :', dLdU)
        #print('dLdW :', dLdW)
        #print('dLdU_ :', dLdU_)
        #print('dLdW_ :', dLdW_)
        ...
        self.mnew_V = self.beta1 * self.mnew_V + (1 - self.beta1) * dLdnew_V
        self.mnew_W1 = self.beta1 * self.mnew_W1 + (1 - self.beta1) * dLdnew_W1
        self.mnew_W2 = self.beta1 * self.mnew_W2 + (1 - self.beta1) * dLdnew_W2
        ...
        self.vnew_V = self.beta2 * self.vnew_V + (1 - self.beta2) * (dLdnew_V ** 2)
        self.vnew_W1 = self.beta2 * self.vnew_W1 + (1 - self.beta2) * (dLdnew_W1 ** 2)
        self.vnew_W2 = self.beta2 * self.vnew_W2 + (1 - self.beta2) * (dLdnew_W2 ** 2)
        ...
        self.new_V += -learning_rate * self.mnew_V / (np.sqrt(self.vnew_V) + self.eps)
        self.new_W1 += -learning_rate * self.mnew_W1 / (np.sqrt(self.vnew_W1) + self.eps)
        self.new_W2 += -learning_rate * self.mnew_W2 / (np.sqrt(self.vnew_W2) + self.eps)
...
losses = model.train_with_sgd(X_train, y_train, nepoch=30, evaluate_loss_after=1)
