...
class LSTM_attention(nn.Module):
    ...
    def numpy_sdg_step(self, x, y, y_, learning_rate):
    ...
    def train_with_batch(self, X_train, y_train, y__train, learning_rate=0.0025, nepoch=100, evaluate_loss_after=5):
            ...
            for i in range(total_step):
                ...
                if i == total_step - 1:
                    self.Loss += self.numpy_sdg_step(X_train[i * batch_size:],
                                                     y_train[i * batch_size:], y__train[i * batch_size:], learning_rate)
                else:
                    self.Loss += self.numpy_sdg_step(X_train[i * batch_size:(i + 1) * batch_size],
                                                     y_train[i * batch_size:(i + 1) * batch_size],
                                                     y__train[i * batch_size:(i + 1) * batch_size], learning_rate)
                Loss_len += 1
...
last_loss = model.train_with_batch(X_train, y_train, y__train, nepoch=300)
...
save('[lstm_attention]view_sentence=1024 batch_size=32 epoch=300.txt')