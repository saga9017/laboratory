...
class multi_initialized_model(nn.Module):
    ...
    def multi_train(self, batch):
        ...
        for i in range(Integration_number):
            ...
            min_last_loss = 1000
            if i > 0:
                optimal_optimizer = copy.deepcopy(optimizers[optimal_model_index])
                optimal_model = copy.deepcopy(self.models[optimal_model_index])
                optimal_params = copy.deepcopy(self.models[optimal_model_index]).named_parameters()
                optimal_params_list = []
                for name, param in optimal_params:
                    optimal_params_list.append((name, param))

            optimal_model_index = 0
            optimizers = []
            interval = 2e-3 / (Integration_object - 1)
            ...
            for index, model_in in enumerate(self.models):
                ...
                if i > 0:
                    ...
                    for param in model_in.parameters():
                        param.data.copy_(param.data-1e-3+interval*index)
