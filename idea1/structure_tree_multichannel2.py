#idea1_multichannel2 : residual, cut-off, multichannel
...
class test(nn.Module):
    ...
    def forward(self, x):
        result1, masks1=self.OneChannel_forward(x, self.sublayer1, self.suffle1, self.layer1)
        result2, masks2=self.OneChannel_forward(x, self.sublayer2, self.suffle2, self.layer2)
        result3, masks3 = self.OneChannel_forward(x, self.sublayer3, self.suffle3, self.layer3)

        result=self.cal_symbols(self.symbols, result1, result2, result3)

        result=torch.cat(result, dim=-1)
        result=self.w(result).squeeze(-1)
...
save('[idea1]view_sentence='+str(view_sentence_len)+' batch_size='+str(Batch_size)+' iteration='+str(ITERATION)+' layer1='+str(Layer1)+' layer2='+str(Layer2)+' layer3='+str(Layer3)+'.txt')
