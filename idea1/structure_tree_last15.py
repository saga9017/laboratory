...
Fold = 0
fine_tune_layer=1
divided=int(Layer1/fine_tune_layer)
#####################################################
...
def define_layer(view_depth, hidden, layer, word_dim2):
    ...
    sublayer = [test_sublayer(word_dim2, combination, hidden, view_depth) for _ in range(layer - 1)]
    sublayer.append(test_sublayer(word_dim2, combination, hidden, view_depth))
    sublayer = nn.ModuleList(sublayer)
    suffle = nn.ModuleList([nn.Linear(hidden, hidden)]+([nn.Linear(word_dim2, hidden) for _ in range(layer-1)]))
    for i in suffle:
...
class test(nn.Module):
    ...
    def OneChannel_forward(self, x, sublayer, suffle):
        result = self.input_embedding(x)
        residual = 0
        quotient=num_examples_seen // int(ITERATION * 0.6 / divided)
        if quotient<divided:
            sublayers = sublayer[:fine_tune_layer * (quotient+1) + 1]
            suffles = suffle[:fine_tune_layer * (quotient+1) + 1]
        else:
            sublayers = sublayer
            suffles = suffle


        for index, (i, j) in enumerate(zip(sublayers, suffles)):
            result = j(result)
            result= i(result)
            if index%2==0:
                result += residual
    ...
    def encoding(self, x):
        result = self.OneChannel_forward(x, self.sublayer1, self.suffle1)
