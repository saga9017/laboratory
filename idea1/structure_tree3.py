...
class test(nn.Module):
    ...
    def optimizing(self, batch, number_seen, optimizer):
        ...
        optimizer.zero_grad()
        loss, masks=self.cal_loss(torch.tensor(batch[0]).to(device),torch.tensor(batch[2]).to(device))
        loss.backward()
...
save('[idea1]view_sentence='+str(view_sentence_len)+' batch_size='+str(MAX_TOKEN)+' iteration='+str(ITERATION)+'.txt')
