...
MAX_OUTPUT = 30
Fold = 0
Integration_object=10
Integration_number=4
#####################################################
...
class test(nn.Module):
    ...
    def optimizing(self, batch, number_seen, optimizer):
        ...
        optimizer.zero_grad()
        loss = self.cal_loss(torch.tensor(batch[0]).to(device),torch.tensor(batch[1]).to(device), torch.tensor(batch[2]).to(device))
        loss.backward()
    ...
    def train_with_batch(self, batch, iteration=0, Num_examples_seen=1):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(batch)):
                ...
                Loss_len += 1
                if num_examples_seen % (len(batch) * nepoch)==0:
...
class multi_initialized_model(nn.Module):
    def __init__(self):
        ...
        self.model_integration=test(input_lang.n_words, output_lang.n_words)

    def multi_train(self):
        view_iteration = int(ITERATION / Integration_number)
        for i in range(Integration_number):
            last_losses=[]
            print('Integration_number :', i)
            for index, j in enumerate(self.models):
                print('model :', index)
                last_losses.append(j.train_with_batch(batch, view_iteration, i*view_iteration+1))

            standard={}
            for model in self.models:
                for index, param in enumerate(model.parameters()):
                    if index not in standard:
                        standard[index]=param/Integration_object
                    else:
                        standard[index]+=param/Integration_object


            interval = 2e-8 / (Integration_object - 1)
            for index2, model in enumerate(self.models):
                for index, param in enumerate(model.parameters()):
                    param.data+=-param.data+standard[index]-1e-8+interval*index2
...
save('[idea1_last10]view_sentence=' + str(view_sentence_len) + ' batch_size=' + str(Batch_size) + ' iteration=' + str(
