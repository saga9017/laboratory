...
class test_sublayer(nn.Module):
    ...
    def forward(self, x):
        ...
        re_mask=(self.w7 <= 1e-8) & (self.w7 >= -1e-8)
        mask = 1 - re_mask.float()
        if num_examples_seen> ITERATION*0.2:
            new_w = self.w7 * mask
        else:
            new_w = self.w7
...
class test(nn.Module):
    ...
    def last_combination(self, results):
        combination_maker1 = make_combination(len(results))
        combination = combination_maker1()

        for index1, i in enumerate(combination):
            output=0
            for index2, (a, b) in enumerate(zip(i, results)):
                if a == '0':
                    output = output + b
                    output = self.layernorm1[index1 * self.view_depth + index2](output)
                elif a == '1':
