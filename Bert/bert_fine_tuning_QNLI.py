...
with open('glue_data/QNLI/train.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1=' '.join(row.split('\t')[1].replace('(', '').replace(')', '').split())
        s2=' '.join(row.split('\t')[2].replace('(', '').replace(')', '').split())
        label=row.split('\t')[-1].replace('\n', '')
        QNLI_train.append([s1, s2, label])

# sentences of test data
with open('glue_data/QNLI/dev.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1 = ' '.join(row.split('\t')[1].replace('(', '').replace(')', '').split())
        s2 = ' '.join(row.split('\t')[2].replace('(', '').replace(')', '').split())
        label = row.split('\t')[-1].replace('\n', '')
        QNLI_test.append([s1, s2, label])
