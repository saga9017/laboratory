...
with open('glue_data/MNLI/train.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1=' '.join(row.split('\t')[4].replace('(', '').replace(')', '').split())
        s2=' '.join(row.split('\t')[5].replace('(', '').replace(')', '').split())
        label=row.split('\t')[-2]
        MNLI_train.append([s1, s2, label])

# sentences of test data
with open('glue_data/MNLI/dev_matched.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1 = ' '.join(row.split('\t')[4].replace('(', '').replace(')', '').split())
        s2 = ' '.join(row.split('\t')[5].replace('(', '').replace(')', '').split())
