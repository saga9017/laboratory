...
with open('glue_data/MRPC/train.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1=' '.join(row.split('\t')[3].replace('(', '').replace(')', '').split())
        s2=' '.join(row.split('\t')[4].replace('(', '').replace(')', '').split())
        label=row.split('\t')[0]
        MNLI_train.append([s1, s2, int(label)])

# sentences of test data
with open('glue_data/MRPC/dev.tsv', 'r', encoding='latin-1') as f:
    f.readline()
    for row in f.readlines():
        s1 = ' '.join(row.split('\t')[3].replace('(', '').replace(')', '').split())
        s2 = ' '.join(row.split('\t')[4].replace('(', '').replace(')', '').split())
        label = row.split('\t')[0]
        MNLI_test.append([s1, s2, int(label)])
